...
class TestMaxAutotune(TestCase):
    ...
    def test_deffered_layout_constraint_mm_cat_mm(self):
        ...
        N = 160
        P = 96
        t1 = torch.randn(B, M, K, device="cuda", dtype=torch.float16)
        t2 = torch.randn(B, K, N, device="cuda", dtype=torch.float16)
        t3 = torch.randn(B, P, N, device="cuda", dtype=torch.float16)
        t4 = torch.randn(B, M, K, device="cuda", dtype=torch.float16)
